...
from libs.hand_recon.mano import MANO
...
class Config:
    ...
    def __init__(self, args=None):
        ...
        self.VIDEO_ROOT = getattr(args, 'video_root', '../assets/debug_eval/wm-h/videos')
        self.LABEL_ROOT = getattr(args, 'label_root', '../assets/debug_eval/wm-h/annotations')
        self.SAVE_PATH = getattr(args, 'save_path', '../output_videos/visualize')
        ...
        self.MODES = getattr(args, 'modes', 'cam,first')
        self.HIDE_LEFT = getattr(args, 'hide_left', False)
        self.HIDE_RIGHT = getattr(args, 'hide_right', False)
        self.NO_RENDER = getattr(args, 'no_render', False)
...
class HandVisualizer:
    ...
    def __init__(self, config: Config, render_gradual_traj: bool = False):
        ...
        self.all_modes = [mode.strip() for mode in self.config.MODES.split(',') if mode.strip()]
        if self.render_gradual_traj:
            self.all_modes = ['cam', 'full', 'first']

        # Initialize MANO and faces only when mesh rendering is requested.
        self.mano = None
        self.faces_left = None
        self.faces_right = None
    ...
    def process_episode(self, episode_name: str):
            ...
            if self.config.NO_RENDER:
                ...
                for frame_idx, frame in enumerate(resize_video_frames):
                    ...
                    final_save_frames.append(add_overlay_text(frame.copy(), overlay_text_full))

                os.makedirs(self.config.SAVE_PATH, exist_ok=True)
                save_to_video(final_save_frames, f'{self.config.SAVE_PATH}/{episode_name}.mp4', fps=self.config.FPS)
                print(f'\nSuccessfully saved preview to {self.config.SAVE_PATH}/{episode_name}.mp4')
                return

            (verts_left_worldspace, left_hand_mask), (verts_right_worldspace, right_hand_mask) = \
                get_hand_labels(episode_info, self.mano, self.config.HIDE_LEFT, self.config.HIDE_RIGHT)
...
def get_hand_labels(episode_info: dict, mano: MANO, hide_left: bool = False, hide_right: bool = False):
    """
    ...
    if hide_left:
        left_hand_mask = np.zeros_like(left_hand_mask)
    ...
    if hide_right:
        right_hand_mask = np.zeros_like(right_hand_mask)
