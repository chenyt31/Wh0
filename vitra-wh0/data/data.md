...
```bash
uv pip install --no-build-isolation git+https://github.com/facebookresearch/pytorch3d.git@stable#egg=pytorch3d  
```
...
```bash
uv pip install projectaria_tools smplx
uv pip install --no-build-isolation git+https://github.com/mattloper/chumpy#egg=chumpy
```
### MANO Hand Model
...
```bash
python data/demo_visualization_epi.py --video_root ../assets/debug_eval/wm-h/videos --label_root ../assets/debug_eval/wm-h/annotations --save_path ../output_videos/visualize --mano_model_path MANO_MODEL_PATH --render_gradual_traj
```
