...
class FrameDataset(Dataset):
    def __init__(self, dataset_folder, dataset_name,
        ...
        self.data_type = data_type  # 'human' or 'robot'
        ...
        elif dataset_name == 'WM-H':
            annotation_file = os.path.join(dataset_folder, "Annotation/WM-H/episode_frame_index.npz")
            label_folder = os.path.join(dataset_folder, "Annotation/WM-H/episodic_annotations")
            statistics_path = os.path.join(dataset_folder, "Annotation/statistics/WM-H_angle_statistics.json")
            video_root = os.path.join(dataset_folder, 'Video/WM-H_root')
        elif dataset_name == 'hoi4d':
            annotation_file = os.path.join(dataset_folder, "Annotation/hoi4d/episode_frame_index.npz")
            label_folder = os.path.join(dataset_folder, "Annotation/hoi4d/episodic_annotations")
            statistics_path = os.path.join(dataset_folder, "Annotation/statistics/WM-H_angle_statistics.json")
            video_root = os.path.join(dataset_folder, 'Video/hoi4d_root')
        elif dataset_name == 'hot3d':
            annotation_file = os.path.join(dataset_folder, "Annotation/hot3d/episode_frame_index.npz")
            label_folder = os.path.join(dataset_folder, "Annotation/hot3d/episodic_annotations")
            statistics_path = os.path.join(dataset_folder, "Annotation/statistics/WM-H_angle_statistics.json")
            video_root = os.path.join(dataset_folder, 'Video/hot3d_root')
        ...
        if statistics_path_override:
            statistics_path = os.path.expanduser(statistics_path_override)
    ...
    def post_transform(self, data):
        ...
        return_dict = dict(
            ...
            domain_label=0 if self.data_type == 'robot' else 1,
        )
...
class MultipleWeightedDataset(Dataset):
    ...
    @staticmethod
    def save_mixed_dataset_statistics(dataset_folder, data_mix, action_type, data_statistics, data_type='human'):
        """Save dataset statistics to JSON file.
        
        Args:
            dataset_folder: Folder to save the statistics
            data_mix: Name of the data mixture
            action_type: Type of action representation
            data_statistics: Statistics dictionary
            data_type: 'human', 'robot', or 'mixed'
        """
        ...
        # Save to local file
        suffix = '' if data_type == 'human' else f'_{data_type}'
        with open(os.path.join(dataset_folder, f"{data_mix}_{action_type}_weighted_statistics{suffix}.json"), "w") as f:
    ...
    def load_datasets(cls, dataset_folder, data_mix,
        ...
        if isinstance(data_statistics, dict) and data_statistics.get('data_type') == 'mixed':
            # Save separate statistics for human and robot
            if 'human' in data_statistics:
                cls.save_mixed_dataset_statistics(dataset_folder, data_mix, action_type, data_statistics['human'], data_type='human')
            if 'robot' in data_statistics:
                cls.save_mixed_dataset_statistics(dataset_folder, data_mix, action_type, data_statistics['robot'], data_type='robot')
            
            # Each dataset sets its own global_data_statistics based on data_type
            for dataset in datasets:
                if dataset.data_type == 'human' and 'human' in data_statistics:
                    dataset.episodic_dataset_core.set_global_data_statistics(data_statistics['human'])
                elif dataset.data_type == 'robot' and 'robot' in data_statistics:
                    dataset.episodic_dataset_core.set_global_data_statistics(data_statistics['robot'])
        else:
            # Single data type - save and set as before
    ...
    def weighted_average_statistics(datasets, weights):
        ...
        Handles both human and robot data types with different dimensions.

        For human data:
            - state_dim: 61 (6 + 10 betas + 45 hand pose)
            - action_dim: 51 (6 + 45 hand pose)
        For robot data:
            - state_dim: 18 (6 EEF + 12 joints)
            - action_dim: 18 (6 EEF + 12 joints)

        This function returns statistics in the original dimensions for each dataset type,
        allowing each dataset to use its own statistics for normalization.
        When multiple dataset types are present, it creates separate statistics for each type.
        ...
        Returns:
            ...
            For single data_type: returns statistics in original dimensions.
            For mixed data_types: returns a dict with 'human' and 'robot' keys containing
            their respective merged statistics.
        """
        ...
        # Separate datasets by data_type
        human_datasets = []
        robot_datasets = []
        human_weights = []
        robot_weights = []
        ...
        for ds, weight in zip(datasets, norm_weights):
            if ds.data_type == 'human':
                human_datasets.append(ds)
                human_weights.append(weight)
            elif ds.data_type == 'robot':
                robot_datasets.append(ds)
                robot_weights.append(weight)

        # If only one data type, return statistics in original dimensions
        if (human_datasets and not robot_datasets) or (not human_datasets and robot_datasets):
            # Use the first dataset's data_type for all
            target_data_type = human_datasets[0].data_type if human_datasets else robot_datasets[0].data_type
            target_datasets = human_datasets if human_datasets else robot_datasets
            target_weights = human_weights if human_datasets else robot_weights

            # Calculate weighted average
            merged_statistics = {}
            keys = target_datasets[0].episodic_dataset_core.data_statistics.keys()

            for key in keys:
                total = None
                for ds, weight in zip(target_datasets, target_weights):
        ...
        # Mixed data types: return separate statistics for each type
        # This allows each dataset to use its own statistics for normalization
        result = {
            'data_type': 'mixed',
        }
        ...
        if human_datasets:
            human_raw_weights = np.array([
                len(ds.episodic_dataset_core) * w
                for ds, w in zip(human_datasets, human_weights)
            ], dtype=np.float64)
            human_norm_weights = human_raw_weights / human_raw_weights.sum()

            human_stats = {}
            keys = human_datasets[0].episodic_dataset_core.data_statistics.keys()

            for key in keys:
                total = None
                for ds, weight in zip(human_datasets, human_norm_weights):
                    value = ds.episodic_dataset_core.data_statistics[key]
                    if total is None:
                        total = weight * value
                    else:
                        total += weight * value
                human_stats[key] = total

            result['human'] = human_stats

        # Compute robot statistics
        if robot_datasets:
            robot_raw_weights = np.array([
                len(ds.episodic_dataset_core) * w
                for ds, w in zip(robot_datasets, robot_weights)
            ], dtype=np.float64)
            robot_norm_weights = robot_raw_weights / robot_raw_weights.sum()

            robot_stats = {}
            keys = robot_datasets[0].episodic_dataset_core.data_statistics.keys()

            for key in keys:
                total = None
                for ds, weight in zip(robot_datasets, robot_norm_weights):
                    value = ds.episodic_dataset_core.data_statistics[key]
                    if total is None:
                        total = weight * value
                    else:
                        total += weight * value
                robot_stats[key] = total

            result['robot'] = robot_stats
