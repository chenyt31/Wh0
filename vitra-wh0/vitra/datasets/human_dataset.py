...
class EpisodicDatasetCore(object):
    ...
    def _prepare_side_window(self, side_dict,
        ...
        kept_frames = np.array(side_dict['kept_frames'])
    ...
    def _grab_window_images(self,
        ...
        decode_table = np.array(epi['video_decode_frame'])                      # (T,)
        T            = len(decode_table)
        ...
        try:
            video_name = episode_id.split("_ep_")[0].split(f"{os.path.basename(self.video_root).replace('_root', '')}_")[1] + '.mp4'
        except:
            video_name = episode_id.split("_ep_")[0].split("WM-H_")[1] + '.mp4'
        if self.clip_len is not None:
            part_idx = frame_in_video // self.clip_len # clip_len = 2000
            frame_in_part = frame_in_video % self.clip_len
            video_path = self._resolve_video_path(dataset_name, video_name, part_idx)
            decode_ids = [frame_in_part]
        else:
            video_path = self._resolve_video_path(dataset_name, video_name)
    ...
    def _build_instruction(
        ...
        if main_type == 'both':
            main_type = 'right'
        if not text_clip.get(main_type):
            if text_clip.get('right'):
                main_type = 'right'
            elif text_clip.get('left'):
                main_type = 'left'
            else:
                text_clip = {
                    'left': [('None.', (0, epi_len - 1))],
                    'right': [('None.', (0, epi_len - 1))],
                }
                main_type = 'right'
    ...
    def transform_trajectory(
        ...
        expected_action_len = self.action_future_window_size
        if expected_action_len > 0 and action_np.shape[0] > expected_action_len:
            action_np = action_np[:expected_action_len]
            sample_dict["action_mask"] = sample_dict["action_mask"][:expected_action_len]
